...
func clean() {
	allManifestsData, err := GetAllData(config.BoltDB, "manifests")
	if err != nil {
	...
	for _, m := range allManifestsData {
		...
		if now.After(oneWeekLater) {
			err = DeleteKey(config.BoltDB, "manifests", manifest.FileId)
			if err != nil {
				log.Println("[Clean] - Error: ", err)
			}
		}
	...
	allActiveNodes, err := GetAllKeys(config.BoltDB, "active_nodes")
	if err != nil {
		log.Println("[Clean] - Error: ", err)
		return
	}
	for _, a := range allActiveNodes {
		...
		if found {
			err = DeleteKey(config.BoltDB, "active_nodes", a)
			if err != nil {
				log.Println("[Clean] - Error: ", err)
			}
		}
