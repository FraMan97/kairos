...
func GenerateKeyPair() error {
	log.Println("[KeyManagement] - Generating Key pair...")
	keysDir := "../keys"
	...
	log.Println("[KeyManagement] - Keys generated successfully!")
	log.Println("[KeyManagement] - Private key: private_key.pem")
	log.Println("[KeyManagement] - Public key: public_key.pem")
...
func GetPublicKey() ([]byte, error) {
	privateKeyPEM, err := os.ReadFile(config.PublicKeyDir)
	if err != nil {
...
func DecryptGCM(data, key []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("gcm.Open error (wrong key or corrupted data): %v", err)
	}
